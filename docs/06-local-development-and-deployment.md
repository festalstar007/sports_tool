...
`0002_manual_activities.sql` 会保留现有截图记录，新增 `source_type`，并允许手动记录的 `import_id` 为 `NULL`。
...
部署完成后，用手机执行一次完整流程：登录、上传跑步截图、校对、保存、查看历史和曲线，再对步行截图重复一次。随后创建一条手动记录，验证三段式时长输入、派生指标、详情编辑和删除。
